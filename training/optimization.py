...
import functools
from optax._src import numerics, wrappers, transform
...
def construct_optimizer(opt_config, return_chainables=False):
    chainables = []
    # clip_norm = opt_config.get('global_max_norm', None)
    # if clip_norm:
    #     chainables = [optax.clip_by_global_norm(clip_norm)]
    # optim = opt_config.get('optim', 'adamw').lower()
    # if optim == 'adamw':
    #     opt = scale_by_bfloat16_adam(b1=opt_config.get('beta_1', 0.9),
    #                                  b2=opt_config.get('beta_2', 0.98),
    #                                  eps=opt_config.get('eps', 1e-8),
    #                                  use_bfloat16=opt_config.get('use_bfloat16_optim', True),
    #                                  do_bias_correction=opt_config.get('do_bias_correction', False),
    #                                  )
    #     chainables += [opt]
    # elif optim == 'sgd':
    #     opt = transform.trace(
    #         decay=opt_config.get('momentum', 0.9),
    #         nesterov=opt_config.get('nesterov', False),
    #         accumulator_dtype=jnp.bfloat16 if opt_config.get('use_bfloat16_optim', True) else None,
    #     )
    #     chainables += [opt]
    # else:
    ...
    learning_rate_fn = optax.warmup_cosine_decay_schedule(
        ...
        warmup_steps = opt_config['num_warmup_steps'],
        decay_steps = opt_config['num_train_steps'],
        end_value = opt_config['end_learning_rate'],
    )
    chainables += [
        optax.clip(max_delta = 1.0),
        optax.adamw(
            learning_rate = learning_rate_fn,
            b1=opt_config.get('beta_1', 0.9),
            b2=opt_config.get('beta_2', 0.98),
            weight_decay=1e-4,
            )
    ]
...
def construct_train_state(opt_config, models, params, rng_keys, return_chainables=False):
    """
    :param optimizer_params: Dict like
        {
      learning_rate: 0.001
      num_train_steps: 93600 # 300 epochs
    ...
    txs = {}
    for k, v in opt_config.items(): txs[k] = construct_optimizer(v)

    state = TrainState_v2.create(
        models=models,
        params=params,
        frozen_params={'g': flax.core.freeze({}), 'd': flax.core.freeze({})},
        txs=txs,
